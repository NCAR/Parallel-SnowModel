...
! SnowTran-3D variables.
  ...
  real windspd_min,tabler_dir,slope_adjust,forcing_dx

  real, allocatable :: topo_land(:,:)
  real, allocatable :: tabler_nn(:,:)
  real, allocatable ::  tabler_nn_orig(:,:)
  real, allocatable ::  tabler_ss(:,:)
  real, allocatable ::  tabler_ss_orig(:,:)
  real, allocatable ::  tabler_ee(:,:)
  real, allocatable ::  tabler_ee_orig(:,:)
  real, allocatable ::  tabler_ww(:,:)
  real, allocatable ::  tabler_ww_orig(:,:)
  real, allocatable ::  tabler_ne(:,:)
  real, allocatable ::  tabler_ne_orig(:,:)
  real, allocatable ::  tabler_se(:,:)
  real, allocatable ::  tabler_se_orig(:,:)
  real, allocatable ::  tabler_sw(:,:)
  real, allocatable ::  tabler_sw_orig(:,:)
  real, allocatable ::  tabler_nw(:,:)
  real, allocatable ::  tabler_nw_orig(:,:)
...
! MicroMet variables.
  ...
  real, allocatable ::  alpha_grid(:,:)  ! alpha angle from snowmodel grid to earth relative
...
! End EnBal variables.
  ...
  integer max_layers,multilayer_snowpack,k,nz_max
...
! Data assimilaion (precipitation and melt) factor variables.
  real, allocatable ::  corr_factor(:,:,:)
  integer, allocatable :: icorr_factor_index(:)
...
! Code profiling timing variables.
  ...
  integer*8 :: snowtran_comm_start,snowtran_comm_end,total_snowtran_comm_time
! End Code profiling timing variables.
contains
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!

  subroutine allocate_arrays(nx,ny,me,max_iter,nz_max)
    implicit none

    integer, intent(in) :: nx,ny,me,max_iter,nz_max
    ...
    allocate(tabler_nn_orig(nx,ny),source=zero_real)
    allocate(tabler_ss_orig(nx,ny),source=zero_real)
    allocate(tabler_ee_orig(nx,ny),source=zero_real)
    allocate(tabler_ww_orig(nx,ny),source=zero_real)
    allocate(tabler_ne_orig(nx,ny),source=zero_real)
    allocate(tabler_nw_orig(nx,ny),source=zero_real)
    allocate(tabler_se_orig(nx,ny),source=zero_real)
    allocate(tabler_sw_orig(nx,ny),source=zero_real)
    ! endif
    ...
    allocate( alpha_grid(nx,ny),source=zero_real)  ! alpha angle from snowmodel grid to earth relative
    ...
    allocate(d_canopy_int(nx,ny),source=zero_real)
    ...
    allocate(windspd_2m_grid(nx,ny),source=zero_real)
    ...
    ! if (multilayer_snow.eq.0 .and. icond_flag.eq.0 .and. seaice_run.lt.0.9) then
    !   continue
    ! else
    ...
    flux_layer = 0.0
    ! endif
    allocate( vars(nx,ny,n_print_vars))
    vars = 0.0
    ! if (seaice_run.gt.0.9) then
    allocate(seaice_conc(nx,ny))
    seaice_conc = 0.0
    ! endif

    allocate( corr_factor(nx,ny,max_iter+1))
    corr_factor = 0.0
    
    allocate( icorr_factor_index(max_iter))
