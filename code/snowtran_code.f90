...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
     ...
     &  snowtran_comm_end,total_snowtran_comm_time,nz)

#ifdef SERIAL
      use serial_module, only: buff_index,local_buffer_1D,&
     &  synchronization,l_vwind_north,l_vwind_south,&
     &  vwind_buffer
#elif CAF
      use caf_module, only: buff_index,local_buffer_1D,&
     &  synchronization,l_vwind_north,l_vwind_south,&
     &  vwind_buffer
#endif
     ...
     &  sum_sfcsublim(nx,ny)
      ...
      real swed_layer_z(nz)
      real swed_layer(nx,ny,nz)
      real snod_layer(nx,ny,nz)
      real ro_layer(nx,ny,nz)
      real T_old(nx,ny,nz)
...
! Added variables.
      ...
      character*9 fmt
      integer*8 snowtran_comm_start,snowtran_comm_end,&
     &  total_snowtran_comm_time
      integer new_j,me,np,global_ny,nz
...
!   have blowing snow.
      ...
      if (me.eq.0) then
        call System_Clock(snowtran_comm_start)
      endif
! Call "sync all" equivalent prior to using coarrays.
      call synchronization()
! Create wind_buffer using coarrays.
      call local_buffer_1D(vwind_grid,vwind_buffer,l_vwind_north,&
     &  l_vwind_south,nx,buff_index)

      if (me.eq.0) then
        call System_Clock(snowtran_comm_end)
        total_snowtran_comm_time = total_snowtran_comm_time +&
     &    (snowtran_comm_end - snowtran_comm_start)
      endif
...
!         print *,'         Saltation'
     ...
     &      ro_air,gravity,vegsnowd_xy,snow_d,Qsalt_max,Qsalt_maxu,&
     &      Qsalt_maxv,deltay,Qsalt_u,Qsalt_v,uwind_grid,vwind_grid,&
     &      xmu,soft_snow_d,bc_flag,vwind_buffer,buff_index,&
     &      snowtran_comm_start,snowtran_comm_end,&
     &      total_snowtran_comm_time)
...
!   sublimation.
     ...
     &    undef,Qsalt_u,Qsalt_v,Qsusp_u,Qsusp_v,Qsubl,dh_salt,&
     ...
     &    curve_len_scale,vwind_buffer,buff_index,&
     &    me,global_ny,np,snowtran_comm_start,&
     &    snowtran_comm_end,total_snowtran_comm_time)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
     ...
     &  sum_trans,swe_depth,snow_depth,ro_snow_grid,&
     &  dh_subgrid,tabler_dir,iter,slope_adjust,&
     &  curve_len_scale,vwind_buff,buff_index,&
     &  me,global_ny,np,snowtran_comm_start,&
     &  snowtran_comm_end,total_snowtran_comm_time)

#ifdef SERIAL
      use serial_module, only: summation
#elif CAF
      use caf_module, only: summation
#endif
      implicit none
...
! Added variables.
      integer*8 snowtran_comm_start,snowtran_comm_end,&
     &  total_snowtran_comm_time
      real vwind_buff(nx,buff_index)
      integer new_j,buff_index
      integer np,me,global_ny
...
! SALTATION
     ...
     &    Qsalt_v,dh_salt_u,dh_salt_v,ro_snow,dt,vegsnowd_xy,snow_d,&
     &    soft_snow_d,snow_d_dep,soft_snow_d_dep,subgrid_flag,uwind_grid,&
     &    vwind_grid,vwind_buff,buff_index,snowtran_comm_start,&
     &    snowtran_comm_end,total_snowtran_comm_time)
...
! SUSPENSION
     ...
     &    Qsusp_v,dh_susp_u,dh_susp_v,ro_snow,dt,vegsnowd_xy,snow_d,&
     &    soft_snow_d,snow_d_dep,soft_snow_d_dep,subgrid_flag,uwind_grid,&
     &    vwind_grid,vwind_buff,buff_index,snowtran_comm_start,&
     &    snowtran_comm_end,total_snowtran_comm_time)
...
! Parallel summation
          call summation(bs_flux_u)
          call summation(bs_flux_v)
...
! Do the sides.
      if (me.eq.0) then
        ...
        do i=2,nx-1
          snow_tmp(i,j) =&
     &      (snow(i,j) + 0.5 * (snow(i,j+1) + snow(i-1,j) +&
     &      snow(i+1,j)) + 0.3 * (snow(i+1,j+1) + snow(i-1,j+1))) /3.1
        enddo
      endif
...
! Interior.
      ...
      do j=2,ny-1
        snow_tmp(i,j) =&
     &    (snow(i,j) + 0.5 * (snow(i,j-1) + snow(i,j+1) +&
     &    snow(i+1,j)) + 0.3 * (snow(i+1,j-1) + snow(i+1,j+1))) /3.1
      enddo
...
! Left.
        ...
        i = 1
        snow_tmp(i,j) =&
     &    (snow(i,j) + 0.5 * (left_rcv(i) + snow(i,j+1) +&
...
! Right.
        ...
        i = 1
        snow_tmp(i,j) =&
     &    (snow(i,j) + 0.5 * (snow(i,j-1) + right_rcv(i) +&
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
     ...
     &  ro_air,gravity,vegsnowd_xy,snow_d,Qsalt_max,Qsalt_maxu,&
     &  Qsalt_maxv,deltay,Qsalt_u,Qsalt_v,uwind_grid,vwind_grid,&
     &  xmu,soft_snow_d,bc_flag,vwind_buff,vwind_index,&
     &  snowtran_comm_start,snowtran_comm_end,&
     &  total_snowtran_comm_time)

#ifdef SERIAL
      use serial_module, only: me,np,buffer_north,buffer_south,&
     &  synchronization,l_qsalt_buff,l_utau_buff,salt_iter
#elif CAF
      use caf_module, only: me,np,buffer_north,buffer_south,&
     &  synchronization,l_qsalt_buff,l_utau_buff,salt_iter
#endif
     ...
     &  blowby,bc_flag
      ...
      integer npairs,iter,vwind_index
      ...
      real vwind_buff(nx,vwind_index)
...
! Consider WESTERLY winds.
      do j=1,ny
        call get_direction_west(uwind_grid,index_start,&
     &    index_end,npairs,nx,ny,j)
        do k=1,npairs
...
! Consider EASTERLY winds.
      do j=1,ny
        call get_direction_east(uwind_grid,index_start,&
     &    index_end,npairs,nx,ny,j)
        do k=1,npairs
          iend = index_start(k)
...
! Consider SOUTHERLY winds.
      ...
      do iter=1,salt_iter
        ...
        call synchronization()
        if (me.ne.np-1) call buffer_south(Qsalt_v,l_qsalt_buff,ny)
        if (me.ne.np-1) call buffer_south(Utau,l_utau_buff,ny)
        call synchronization()
        if (me.eq.0) then
          call System_Clock(snowtran_comm_end)
          total_snowtran_comm_time = total_snowtran_comm_time +&
     &      (snowtran_comm_end - snowtran_comm_start)
        ...
        do i=1,nx
          call get_direction_south(vwind_buff,index_start,&
     &      index_end,npairs,nx,vwind_index,i)
          do k=1,npairs
            jstart = index_start(k)+1
            jend = index_end(k)
            if (jstart.eq.1 .and. jend.ge.jstart) then
              dUtau = Utau(i,jstart) - l_utau_buff(i)
              if (dUtau.ge.0.0) then
                Qsalt_v(i,jstart) = l_qsalt_buff(i) + scale_NS *&
     &            (Qsalt_maxv(i,jstart) - l_qsalt_buff(i))
              else
                if (l_qsalt_buff(i) .lt. Qsalt_maxv(i,jstart)) then
                  Qsalt_v(i,jstart) = l_qsalt_buff(i)
                else
                  Qsalt_v(i,jstart) =&
     &              max(blowby*l_qsalt_buff(i),Qsalt_maxv(i,jstart))
                endif
              endif
              jstart = jstart + 1
            endif
...
! Consider NORTHERLY winds.
      do iter=1,salt_iter
        ...
        call synchronization()
        if (me.ne.0) call buffer_north(Qsalt_v, l_qsalt_buff)
        if (me.ne.0) call buffer_north(Utau,l_utau_buff)
        call synchronization()
        if (me.eq.0) then
          call System_Clock(snowtran_comm_end)
          total_snowtran_comm_time = total_snowtran_comm_time +&
     &       (snowtran_comm_end - snowtran_comm_start)
        ...
        do i=1,nx
          call get_direction_north(vwind_buff,index_start,&
     &      index_end,npairs,nx,vwind_index,i)
          do k=1, npairs
            jend = index_start(k)
            jstart = index_end(k)-1
            if (jstart.eq.ny .and. jend.le.jstart) then
              dUtau = Utau(i,jstart) - l_utau_buff(i)
              if (dUtau.ge.0.0) then
                Qsalt_v(i,jstart) = l_qsalt_buff(i) + scale_NS *&
     &            (Qsalt_maxv(i,jstart) - l_qsalt_buff(i))
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      ...
      subroutine get_direction_west(uwind_grid,index_start,&
     &  index_end,npairs,nx,ny,new_j)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      ...
      subroutine get_direction_east(uwind_grid,index_start,&
     &  index_end,npairs,nx,ny,new_j)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      ...
      subroutine get_direction_south(vwind_grid,index_start,&
     &  index_end,npairs,nx,ny,i)

#ifdef SERIAL
      use serial_module, only: l_ny, me
#elif CAF
      use caf_module, only: l_ny, me
#endif

      real, intent(in) ::  vwind_grid(nx,ny)
      integer, intent(inout) :: index_start(l_ny), index_end(l_ny)
      integer, intent(out) :: npairs
      integer, intent(in) :: nx,ny,i
      real sign1,sign2
      integer :: j, new_j, start_j,end_j
...
! Sweep looking for SOUTHERLY winds, looking for positive numbers.
      if (vwind_grid(i,start_j).le.0.0) then
        sign1 = -1.0
      else
        sign1 = 1.0
      endif
...
!   the group.
      ...
      if (me.eq.0) then
        if (vwind_grid(i,l_ny).le.0.0) then
          sign1 = -1.0
        else
          sign1 = 1.0
        endif
        ...
        if (vwind_grid(i,l_ny+1).le.0.0) then
          sign1 = -1.0
        else
          sign1 = 1.0
        endif
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      ...
      subroutine get_direction_north(vwind_grid,index_start,&
     &  index_end,npairs,nx,ny,i)

#ifdef SERIAL
      use serial_module, only: l_ny,me,np
#elif CAF
      use caf_module, only: l_ny,me,np
#endif
...
!   the group.
      ...
      if (sign1.lt.0.0) then
        ...
        if (np.gt.1) then
          ...
          elseif (me.gt.0 .and. me.lt.np-1) then
            if (vwind_grid(i,l_ny+2).le.0.0) then
              index_end(npairs) = l_ny+1 !interval goes to right neighbour
            endif
          endif
        endif
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
     ...
     &  uwind_grid,vwind_grid,vwind_buff,buff_index,&
     &  snowtran_comm_start,snowtran_comm_end,&
     &  total_snowtran_comm_time)
...
#ifdef SERIAL
      use serial_module, only: me,np,buffer_north,buffer_south,&
     &  synchronization,l_qsalt_buff,salt_iter
#elif CAF
      use caf_module, only: me,np,buffer_north,buffer_south,&
     &  synchronization,l_qsalt_buff,salt_iter
#endif
      ...
      integer i,j,nx,ny
      integer k,istart,iend,jstart,jend,new_lny,buff_index
...
! Consider SOUTHERLY winds.
        do i=1,nx
          call get_direction_south(vwind_buff,index_start,&
     &      index_end,npairs,nx,buff_index,i)
          do k=1,npairs
            jstart = index_start(k)+1
            jend = index_end(k)
            if (jstart.eq.1 .and. jend.ge.jstart) then
! Use buffer.
              dQsalt = Qsalt_v_tmp(i,jstart) - l_qsalt_buff(i)!Qsalt_v(i,j-1)
...
!   eroded.
              ...
              if (snow_d(i,jstart).gt.snowdmin) then
                if (snow_d(i,jstart) + dh_salt_v(i,jstart).le.snowdmin) then
                  ...
                  Qsalt_v(i,jstart) = l_qsalt_buff(i) -&
     &              dh_salt_v(i,jstart) * ro_snow * deltay / dt
                endif
              else
            ...
            do j=jstart,jend
              dQsalt = Qsalt_v_tmp(i,j) - Qsalt_v(i,j-1)
...
!      integer index_ue(ny,2*nx+1)
!      integer index_uw(ny,2*nx+1)
!      integer index_vn(nx,2*ny+1)
!      integer index_vs(nx,2*ny+1)
...
! integer, parameter :: nx_max_tabler = nx*1000
...
! integer,parameter :: ny_max_tabler=ny*1000
