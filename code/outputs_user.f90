...
! Serial output.
              do k=1,n_print_vars
                if (print_var(k).eq.'y') then
                  open (320+k,&
     &file=output_path_wi_assim(1:i_len_wo)//print_outvars(k)//'.gdat',&
     &            form='unformatted',access='direct',recl=4*nx*ny,&
     &            status='replace')
                endif
              enddo
...
! Parallel output.
              do k=1,n_print_vars
                if (print_var(k).eq.'y') then
                  open (320+k+me,&
     &file=output_path_wi_assim(1:i_len_wo)//print_outvars(k)//&
     &            '_'//TRIM(ADJUSTL(me_string))//'.gdat', &
     &            form='unformatted',access='direct',recl=4*nx*ny,&
     &            status='replace')
                endif
