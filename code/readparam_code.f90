...
!   before the model simulation starts.
     ...
     &    i_param_chars,i_value_chars,icomment_flag,me)
...
! GENERAL MODEL SETUP.
     ...
     &        c_param(1:i_param_chars),me)
            if (igrads_metfile.ne.0 .and. igrads_metfile.ne.1 .and. &
     &        igrads_metfile.ne.2) then
              print *,'igrads_metfile not 0, 1, or 2'
              stop
            endif
          endif

          if (c_param(1:i_param_chars).eq.'netcdf_fpath') then
            ipar_count = ipar_count + 1
            cpar_name(ipar_count) = c_param(1:i_param_chars)
            call char2char(netcdf_fpath,c_value,i_value_chars,&
     &        c_param(1:i_param_chars),me)
          endif
...
! MICROMET MODEL SETUP.
     ...
     &        c_value,c_param(1:i_param_chars),me)
     ...
     &        .and. snowfall_frac.ne.3.0 .and. snowfall_frac.ne.4.0) then
              print *,'snowfall_frac must be = 1.0, 2.0, 3.0, or 4.0'
              stop
            endif
          endif
...
! Set nz_max parameter equal to max_layers + 1
            nz_max = max_layers + 1
          endif
...
! PRINTING OUTPUT FILES.
     ...
     &        i_value_chars,c_param(1:i_param_chars),me)
          endif
...
!   parameters have been defined.
      ...
      if (ipar_flag.ne.0 .and. me.eq.0) then
        print *
        print *
...
!   longer than 80 characters.
          if (i_value_chars.gt.80 .and. me.eq.0) then
            print *
            print *, input_string
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      ...
      subroutine GET_DOT_PAR_PATH_LOCATION(snowmodel_dot_par_fname,me)
