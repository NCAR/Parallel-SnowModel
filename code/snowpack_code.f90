...
!      include 'snowmodel.inc'
  ...
  real flux_layer(nx,max_l_ny,nz_max)
  ...
  real prec_grid(nx,max_l_ny)
  real windspd_grid(nx,max_l_ny)
  real windspd_2m_grid(nx,max_l_ny)
  ...
  real, dimension(nx,max_l_ny) :: ro_nsnow,snow_d,&
       &  runoff, rain,&
       &  sprec, w_balance,&
       &  sum_prec, sum_runoff,&
...
!      include 'snowmodel.inc'
  ...
  do j=1,l_ny
     new_j = global_map(j)
     do i=1,nx
        if (seaice_conc(i,new_j).eq.0.0) then
           canopy_int_old(i,j) = 0.0
           swe_depth_old(i,j) = 0.0
           snow_depth(i,j) = 0.0
           ro_snow_grid(i,j) = ro_snow
