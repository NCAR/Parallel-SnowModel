...
!   corresponds to.
      if (seaice_run.ne.0.0) then
        ...
        if (mod(iter-1,n_tsteps_in_day).eq.0) then
          ...
          read (445,pos=byte_index_r1) ((seaice_conc(i,j),i=1,nx),j=1,ny)
        endif
      endif
...
!   of wind on the top snow layer.
     ...
     &  T_old,KK,snod_layer,A1,A2,windspd_2m,nz_max)
...
! Distribute surface melt and rain precipitation through the snowpack.
     ...
     &  corr_factor,icorr_factor_index,nz_max)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      ...
      SUBROUTINE GETGAMMA(KK,ro_layer,gamma,nz_max)
...
! Check to see whether this sublimation requires a layer reduction.
        CALL REDUCE_LAYERS(swesublim,swed_layer,KK,nz_max)
...
! Check to see whether this melt leads to a reduction in layers.
        CALL REDUCE_LAYERS(swemelt,swed_layer,KK,nz_max)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      ...
      SUBROUTINE REDUCE_LAYERS(swemelt,swed_layer,KK,nz_max)
...
!     CALL GETGAMMA(KK,ro_layer,gamma,nz_max)

! This is the thermal conductivity that depends on grain size.
      CALL GET_THERMAL_CONDUCTIVITY(KK,ro_layer,diam_layer,gamma,nz_max)

      if (KK.gt.1) then

! Update the control volume information.
        CALL GETCV(KK,dy_p,snod_layer,nz_max)
        CALL CV_INFO(dely_p,f_n,y_crds,y_wall,dy_p,KK,nz_max)

! Compute the general equation coefficients.
        CALL GAMMA1(g_b_ns,gamma,f_n,KK,nz_max)
        CALL GE_COEF(aN,aS,aP0,dy_p,dely_p,g_b_ns,dt,KK,&
     &    ro_layer,Cp_snow,nz_max)
...
! Configure the information for the matrix solver.
     ...
     &    dy_p,bc_S,bc_N,Sc,Sp,aN,aS,aP0,KK,nz_max)

! Solve the system of equations.
        CALL TRISOLVE(T_old,A_sub,A_main,A_super,b_vector,KK,nz_max)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      ...
      SUBROUTINE GETCV(KK,dy_p,snod_layer,nz_max)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      ...
      SUBROUTINE CV_INFO(dely_p,f_n,y_crds,y_wall,dy_p,KK,nz_max)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      ...
      SUBROUTINE GAMMA1(g_b_ns,gamma,f_n,KK,nz_max)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      ...
      SUBROUTINE TRISOLVE(x,asub,amain,asuper,b,KK,nz_max)
