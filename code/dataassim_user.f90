...
!   to those in the snowmodel.par file).
...
!   /sm/swe_assim/.  And it is assumed to be called: swe_obs.dat.
!   See below for the details of how this file is to be configured.
...
!   file written to /sm/swe_assim/ that provides a summary of
!   the calculations that were done (see unit=77 in the code
!   below for what is written to this file).  Second, there is a
!   copy of the precipitation correction surfaces that were
!   generated.  This is also placed in /sm/swe_assim/, the file
!   is a GrADS file (called corr_factor.gdat), and there is
...
! The first line is how many years there are in this SnowModel
!   simulation.  Note that these are SnowModel simulation years.
!   So, for example, year 1 might be 1 Sep 2007 - 31 Aug 2008, and
!   year 2 might be 1 Sep 2008 - 31 Aug 2009, etc.
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      ...
      real sprec_ratio(max_iter),smelt_ratio(max_iter)
      real corr_factor(nx,ny,max_iter)
      real areas_mask(nx,ny)

      double precision xmn,ymn
      double precision xstn(nx*ny),ystn(nx*ny)

      integer icorr_factor_index(max_iter)
      integer iobs_rec(max_iter)
...
! Run the data assimilation routines.
     ...
     &    nyears,dt,max_iter,xhour_init,nstns_max)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
     ...
     &  print_inc,dt,xhour_init
      real sprec_ratio(max_iter),smelt_ratio(max_iter)
      real corr_factor(nx,ny,max_iter)
      real swe_tmp(nx,ny),sum_sprec_tmp1(nx,ny),&
     &  sum_sprec_tmp2(nx,ny),grid(nx,ny),&
     &  areas_mask(nx,ny),sum_smelt_tmp1(nx,ny),&
     &  sum_smelt_tmp2(nx,ny)
      real corr_factor_tmp(nx*ny),swe_obs(nx*ny),&
     &  swe_model(nx*ny),sumsprec_model(nx*ny),&
     &  delta_old(nx*ny),obsid(nx*ny),&
     &  sumsmelt_model(nx*ny),obsid_old(nx*ny),&
     &  delta_old_tmp(nx*ny)
!     real corr_offset(nx*ny)

      double precision xmn,ymn
      double precision xstn(nx*ny),ystn(nx*ny)

      integer iobs_rec(max_iter)
      integer ii(nx*ny),jj(nx*ny)
...
!   factors with 1.0's in places outside the local obs influences.
     ...
     &        corr_factor_tmp,beta,iobsint,ifill,grid,nstns_max)
          endif
...
!   .gdat corr_factor file that was generated by this model run.
     ...
     &  nobs_total,nyears)
      ...
      return
      end subroutine data_assimilation
...
! SAVE A VERSION OF THE INDEX THAT THE USER CAN EASILY LOOK AT.
      ...
      return
      end subroutine corr_factor_index
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
     ...
     &  iobsint,nstns2
      ...
      real xmask(nx,ny),grid(nx,ny)
      real corr_factor_tmp(nx*ny),cf_tmp2(nx*ny),&
     &  obsid(nx*ny)

      real var(nstns_max)

      double precision xmn,ymn
      double precision yg(nx,ny),xg(nx,ny)
