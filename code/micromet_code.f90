...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
     ...
     &  max_iter,nstns_max)
...
#ifdef NETCDF
      use netcdf_module, only: model_netcdf_time,get_netcdf_data
#elif NO_NETCDF
      use nonetcdf_module, only: model_netcdf_time,get_netcdf_data
#endif
      implicit none
      ...
      real rain_orig(nstns_max)     ! input values
      real snow_orig(nstns_max)     ! input values
...
! alpha angle from snowmodel grid to earth relative
...
! Added variables.
      integer netcdf_time_index,np,me,global_ny,max_iter
      integer,parameter :: k15 = selected_int_kind(15)
      integer(kind=k15) :: byte_index_r1
      character*80 netcdf_fpath
      integer prefix_sum(np)
      integer nstns_max
...
! Calculate what the current simulation date should be.
     ...
     &  xhour_init,iter,dt,iyear,imonth,iday,xhour,J_day)
      ...
      if (irun_data_assim.eq.1 .and. me.eq.0) then
...
!   valid observations to be interpolated.
     ...
     &    igrads_metfile,iter,me,nstns_max)

      else
        call model_netcdf_time(iyear,imonth,iday,xhour,netcdf_time_index)

        call get_netcdf_data(nstns_orig,Tair_orig,rh_orig,xstn_orig,&
     &    ystn_orig,elev_orig,iyear,imonth,iday,xhour,undef,&
     &    windspd_orig,winddir_orig,prec_orig,isingle_stn_flag,&
...
!   to be adjusted to account for this.
     ...
     &      form='unformatted',access='stream')
        ...
        read (92,pos=byte_index_r1) ((xlon_grid(i,j),i=1,nx),j=1,ny)
...
!       print *,'   solving for relative humidity'
     ...
     &    seaice_run,global_ny,nstns_max)
      endif
...
!       print *,'   solving for wind speed and direction'
     ...
     &    curve_len_scale,seaice_run,alpha_grid,global_ny,me,&
     &    nstns_max)
...
!       print *,'   solving for precipitation'
     ...
     &    seaice_run,rain_orig,snow_orig,global_ny,max_iter,&
     &    nstns_max)
      endif
...
! Added variables.
      real rain_orig(nstns_max) ! input values
      real snow_orig(nstns_max) ! input values
      real rain(nstns_max)      ! input values
      real snow(nstns_max)      ! input values
      real rain_grid(nx,ny)     ! output values
      real snow_grid(nx,ny)     ! output values
      integer new_j             ! new index for global mapping
      integer global_ny         ! ny for global domain
      integer max_iter          ! number of iterations
      real snowfall_frac_grid(nx,ny) ! calculate snowfall_frac
      integer nstns_max         ! maximum number of met stations

      if (snowfall_frac.ne.4.0) then

! Filter through the original input data, and eliminate any
!   missing values.
        call get_good_values1(nstns_orig,xstn_orig,ystn_orig,&
     &    elev_orig,undef,nstns,xstn,ystn,elev,prec_orig,prec,nstns_max)
...
!   missing values.
        call get_good_values1(nstns_orig,xstn_orig,ystn_orig,&
     &    elev_orig,undef,nstns,xstn,ystn,elev,rain_orig,rain,nstns_max)

! Filter through the original input data, and eliminate any
!   missing values.
        call get_good_values1(nstns_orig,xstn_orig,ystn_orig,&
     &    elev_orig,undef,nstns,xstn,ystn,elev,snow_orig,snow,nstns_max)
...
!   the grid.
        call interpolate(nx,ny,deltax,deltay,xmn,ymn,&
     &    nstns,xstn,ystn,rain,dn,rain_grid,undef,ifill,iobsint,&
     &    iyear,imonth,iday,xhour,barnes_lg_domain,n_stns_used,&
     &    k_stn,snowmodel_line_flag,xg_line,yg_line,seaice_run,&
     &    global_ny,nstns_max)
...
!   the grid.
        call interpolate(nx,ny,deltax,deltay,xmn,ymn,&
     &    nstns,xstn,ystn,snow,dn,snow_grid,undef,ifill,iobsint,&
     &    iyear,imonth,iday,xhour,barnes_lg_domain,n_stns_used,&
     &    k_stn,snowmodel_line_flag,xg_line,yg_line,seaice_run,&
     &    global_ny,nstns_max)

! Conditional statement to allow blowing snow conditions when
!   interpolated rain is less than snow.
        do j=1,ny
          do i=1,nx
            if (rain_grid(i,j).lt.snow_grid(i,j)) then
              snow_grid(i,j) = rain_grid(i,j) + snow_grid(i,j)
              rain_grid(i,j) = 0.0
            endif
            prec_grid(i,j) = snow_grid(i,j) + rain_grid(i,j)
            if (prec_grid(i,j).gt.0.0) then
              snowfall_frac_grid(i,j) = snow_grid(i,j) / prec_grid(i,j)
            else
              snowfall_frac_grid(i,j) = 0.0
            endif
          enddo
        enddo
...
! Use WRF rain-snow partitioning.
      elseif (snowfall_frac.eq.4.0) then

        do j=1,ny
          do i=1,nx
            if (prec_grid(i,j).gt.0.0) then
              sprec(i,j) = snowfall_frac_grid(i,j) * prec_grid(i,j)
            else
              sprec(i,j) = 0.0
            endif
          enddo
...
!   with interpolating over the 360/0 direction line.)
...
#ifdef SERIAL
      use serial_module, only: maximum,summation
#elif CAF
      use caf_module, only: maximum,summation
#endif
      implicit none
     ...
     &  delta_topo,alfa1,alfa2
      ...
      real seaice_run
! Added variables.
      real forcing_dx  ! average forcing spacing
      integer me  ! processor identification
      integer global_ny  ! ny for global domain
      real alpha_grid(nx,ny)  ! alpha angle
      integer nstns_max  ! maximum number of stations
...
! Get forcing station spacing.
      call get_forcing_dx(xstn,ystn,nstns,undef,forcing_dx,&
     & nstns_max) 

! Smooth u_grid field.
! gel
      call smooth_array_2d(u_grid,nx,ny,global_ny,deltax,forcing_dx)
! gel
...
! Smooth v_grid field.
! gel
      call smooth_array_2d(v_grid,nx,ny,global_ny,deltax,forcing_dx)
! gel

! Rotate u and v components based on alpha_grid.
      call rotate_wind(v_grid,u_grid,alpha_grid,windspd_min,&
     &  nx,ny,me,iyear,imonth,iday,xhour)
...
!   the atan2 computation.
          ...
          if (abs(v_grid(i,j)).lt.1e-10) v_grid(i,j) = 1e-10
...
!   wind-topography relationships.
     ...
     &  vegsnowd_xy,curve_len_scale,deltax,deltay,global_ny)
...
!   domain-averaged wind direction.
      ...
      do j=1,ny
        ...
        enddo
      enddo
! Summation and maximum functions.
      call summation(u_sum)
      call summation(v_sum)
      call maximum(windspd_flag)
...
!   the atan2 computation.
      ...
      if (abs(v_sum).lt.1e-10) v_sum = 1e-10
...
! Added variables.
      integer global_ny  ! ny for global domain
      integer nstns_max  ! max number of met stations

! Filter through the original input data, and eliminate any
!   missing values.
      call get_good_values1(nstns_orig,xstn_orig,ystn_orig,&
     &  elev_orig,undef,nstns,xstn,ystn,elev,Tair_orig,Tair,nstns_max)
...
#ifdef SERIAL
      use serial_module, only: maximum
#elif CAF
      use caf_module, only: maximum
#endif
      implicit none
...
! Get maximum wslope of global domain.
      call maximum(wslope_max)
...
! Don't do this smoothing if the domain is arbitrarily small.
      if (nx.gt.100 .and. global_ny.gt.100) then
        do k=1,loops_windwt_smoother
          call smoother9_parallel(nx,ny,windwt)
        enddo
      endif
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
     ...
     &  curvature,terrain_slope,slope_az,curve_len_scale)
      ...
      real terrain_slope(nx,ny)
! Added variables.
      integer new_j,new_j_start,new_j_end
...
! Create topo_buffer.
        call topo_buffer_1D(topo, topo_buffer,nx,inc,&
     &    l_topo_buff_s, l_topo_buff_n)

! Compute the curvature.
        do i=1,nx
          do j=1,ny
            new_j = j + new_jj
            curvature(i,j) = (4.0 * topo_buffer(i,new_j) - &
     &        topo_buffer(max(1,i-inc),max(1,new_j-inc)) - &
     &        topo_buffer(min(nx,i+inc),min(neww_lny,new_j+inc)) -&
     &        topo_buffer(min(nx,i+inc),max(1,new_j-inc)) -&
     &        topo_buffer(max(1,i-inc),min(neww_lny,new_j+inc))) /&
     &        (sqrt(2.0) * 16.0 * real(inc) * deltaxy) +&
     &        (4.0 * topo_buffer(i,new_j) -&
     &        topo_buffer(min(nx,i+inc),new_j) -&
     &        topo_buffer(max(1,i-inc),new_j) -&
     &        topo_buffer(i,min(neww_lny,new_j+inc)) -&
     &        topo_buffer(i,max(1,new_j-inc))) /&
...
!   divisions by zero in flat terrain where the curvature is zero.
        ...
        do j=1,ny
          ...
          enddo
        enddo
! Get maximum curve_max of global domain.
        call maximum(curve_max)
...
! Bottom of domain.
              ...
              do j=2,ny
                dzdy(i,j) = (topo_buffer(i,j+1) - topo_buffer(i,j-1)) /&
            &     (2.0 * deltay)
              enddo
! Top of domain.
            elseif(me.eq.np-1) then
              do j=1,ny-1
                new_j = j+inc
                dzdy(i,j) = (topo_buffer(i,new_j+1) - &
     &            topo_buffer(i,new_j-1)) / (2.0 * deltay)
              enddo
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
     ...
     &  global_ny,nstns_max)
      ...
      double precision xstn_tmp(n_stns_used) ! input stn x coords
      double precision ystn_tmp(n_stns_used) ! input stn y coords
      real var_tmp(n_stns_used)  ! input values
...
! Added variables.
      integer global_ny  ! global domain ny
      integer nstns_max  ! max number of met stations
...
!   simulation.
      if (seaice_run.eq.4.0) then
        ...
        if (nstns.ge.2) then
          call get_dn(nx,ny,deltax,deltay,nstns,dn,iobsint,global_ny)
...
! Do the interpolation for this model grid cell.
     ...
     &        nstns,xstn,ystn,var,dn,grid,undef,ifill,nstns_max)

          endif

        elseif (nstns.eq.1) then

          call single_stn(nx,ny,nstns,var,grid,nstns_max)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
     ...
     &  elev_orig,undef,nstns,xstn,ystn,elev,var_orig,var,nstns_max)
...
!   rh, and wind speed and dir to interpolate the winds.
      ...
      integer k
! Added variables.
      integer nstns_max  ! max met stations
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      ...
      subroutine get_dn(nx,ny,deltax,deltay,nstns,dn,iobsint,global_ny)
...
!   is a reasonable value to use in the interpolation.
        dn_max = sqrt(deltax*real(nx) * deltay*real(global_ny)) * &
     &    ((1.0 + sqrt(real(nstns))) / (real(nstns) - 1.0))
        dn_min = sqrt((deltax*real(nx) * deltay*real(global_ny)) / &
     &    real(nstns))
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
     ...
     &  nstns,xstn,ystn,var,dn,grid,undef,ifill,i,j,&
...
!   Meteorology, 22(9), 1487-1503.
      ...
      real anum_1      ! numerator, beyond scanning radius,
      real anum_2      ! for first and second passes
! Added variables.     ! new index for global mapping
      integer new_j
...
! Assign the station value to every grid cell.
      do j=1,ny
        do i=1,nx
          grid(i,j) = var(nstns)
        enddo
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
     ...
     &  iyear,imonth,iday,xhour,undef,var_grid,iter,global_ny,&
     &  nstns_max)
...
!   good data, do the correction/data assimilation.
     ...
     &    nstns,var_obs,delta_var_grid,var_grid,global_ny,nstns_max)
      endif
...
!   missing values.
      call get_good_values1(nstns_orig,xstn_orig,ystn_orig,&
     &  elev_orig,undef,nstns,xstn,ystn,elev,var_orig,var_obs,nstns_max)
...
!   the .par file).
     ...
     &    nstns_max)
      elseif (nstns.eq.1) then
        call single_stn(nx,ny,nstns,delta_var,delta_var_grid,nstns_max)
      endif
...
#ifdef SERIAL
      use serial_module, only: summation,maximum
#elif CAF
      use caf_module, only: summation,maximum
#endif
      implicit none
...
! Read in the u and v arrays for this time step.
     ...
     &    form='unformatted',access='stream')
      ...
      irec = (iter - 1) * 2
      read (66,pos=byte_index_r1) ((uwind_grid(i,j),i=1,nx),j=1,ny)
      read (66,pos=byte_index_r1) ((vwind_grid(i,j),i=1,nx),j=1,ny)
...
!   the atan2 computation.
          ...
          if (abs(vwind_grid(i,j)).lt.1e-10) vwind_grid(i,j) = 1e-10
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      ...
      subroutine rotate_wind(v_grid,u_grid,alpha_grid,windspd_min,&
     &  nx,ny,me,iyear,imonth,iday,xhour)

      implicit none
      real, intent(inout) :: v_grid(nx,ny),u_grid(nx,ny)
      real, intent(in) :: alpha_grid(nx,ny)
      real, intent(in) :: windspd_min,xhour
      integer, intent(in) :: nx,ny,me,iyear,imonth,iday

      real :: winddir_grid_(nx,ny),winddir_spd_(nx,ny)
      real :: pi,deg2rad,rad2deg
      integer :: i,j
...
!   the atan2 computation.
          ...
          winddir_grid_(i,j) = rad2deg * atan2(u_grid(i,j),v_grid(i,j))
          if (winddir_grid_(i,j).ge.180.0) then
            winddir_grid_(i,j) = winddir_grid_(i,j) - 180.0
          else
            winddir_grid_(i,j) = winddir_grid_(i,j) + 180.0
          endif

          winddir_spd_(i,j) = sqrt(u_grid(i,j)**2 + v_grid(i,j)**2)
        enddo
      enddo

! Convert wind_grid to earth relative coordinates in situations
!   where xlat_grid and xlon_grid are provided
      do j=1,ny
        do i=1,nx
          winddir_grid_(i,j) = modulo(((winddir_grid_(i,j) + &
     &      alpha_grid(i,j))+360.0),360.0)
        enddo
      enddo

! Convert back to u and v grid
      do j=1,ny
        do i=1,nx
          u_grid(i,j) = (- winddir_spd_(i,j)) * &
     &      sin(deg2rad * winddir_grid_(i,j))
          v_grid(i,j) = (- winddir_spd_(i,j)) * &
     &      cos(deg2rad * winddir_grid_(i,j))
        enddo
      enddo
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      ...
      subroutine smooth_array_2d(var,nx,ny,global_ny,deltax,forcing_dx)

#ifdef SERIAL
      use serial_module, only: distribute_buffer,partition_smoother,&
     &  localize
#elif CAF
      use caf_module, only: distribute_buffer,partition_smoother,&
     &  localize
#endif
      implicit none
      real var(nx,ny)
      integer nx,ny,global_ny
      real forcing_dx,deltax

      real,allocatable,dimension(:,:):: input,inputtemp
      integer::i,j,startx,endx,starty,endy
      real,allocatable,dimension(:) :: rowsums,rowmeans
      real :: cursum
      integer :: cur_n,curcol,ncols,nrows,smooth_index,windowsize

! Create index and indexing arrays specific to forcing_dx and windowsize
!  at this timestep.
      call partition_smoother(nx,ny,forcing_dx,deltax,smooth_index,&
     &  windowsize)

! Allocate arrays.
      allocate(input(nx,smooth_index))
      allocate(inputtemp(nx,smooth_index))
      allocate(rowsums(nx))
      allocate(rowmeans(nx))

! Halo-exchange to distribute windowsize of wind grids to neighboring
!   processors.
      call distribute_buffer(var,input,nx,ny)

! Make a copy so we always use the unsmoothed data when computing the
!   smoothed data.
      inputtemp = input

! Parallelize over a slower dimension (not the slowest because it is
!   MUCH easier this way) as long as the inner loops (the array
!   operations) are over the fastest dimension we are mostly OK.
      nrows = windowsize * 2 + 1
      ncols = windowsize * 2 + 1
      do j=1,smooth_index

! So we pre-compute the sum over rows for each column in the current
!   window.
        rowsums = 0
        do i = -1*windowsize, windowsize, 1
          starty = max(1,min(smooth_index,j+i))
          rowsums = rowsums + inputtemp(1:nx,starty)
        enddo

! For a parallel algorithm (in 2D) we recompute the row sums for
!   every y.
        rowmeans = rowsums / nrows
        cursum = sum(rowmeans(1:min(windowsize,nx))) + rowmeans(1) * &
     &    (windowsize + 1)
...
! Localize buffer to local variable shape.
      call localize(input,var,nx,ny) ! Reindex wind field

! Deallocate arrays.
      deallocate(rowmeans,rowsums)
      deallocate(inputtemp,input)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      ...
      subroutine get_forcing_dx (xstn,ystn,nstns,undef,dist_avg,&
     &  nstns_max)

! Compute average distance between stations. This subroutine assumes
!   that:
!   a) station data (ie non reanalysis met datasets) includes nearby
!      stations that are intended to be used in the simulation.  In
!      other words, the stations are nearby the SnowModel grid and
!      intended to be used in the simulation.
!   b) reanalysis datasets have a relatively constant distance
!      inbetween met stations.  This is why we only use 100 to
!      calculate the average distance.

      implicit none
      double precision xstn(nstns_max)
      double precision ystn(nstns_max)
      real dist,dist_ind,dist_sum,dist_avg,undef
      integer i,j,nstns,n_stns_used,n_stns_loop
! Added variables.
      integer nstns_max

! Calculate number of stations in timestep.
      n_stns_used = 0
      do i=1,nstns
        if (ystn(i).ne.undef) then
          n_stns_used = n_stns_used + 1
        endif
      enddo

! Depending on the number of met stations in file being used.
      if (n_stns_used.gt.100) then
        n_stns_loop = 100
      else
        n_stns_loop = n_stns_used
      endif

! Calculate distance between closest.
      dist_sum = 0.0
      do i = 1,n_stns_loop
! Find nearest stn.
        dist = 10000000.0
        do j = 1,n_stns_loop
          if (i.ne.j) then
            dist_ind = sqrt((ystn(i) - ystn(j))**2 + &
     &        ((xstn(i) - xstn(j))**2))
            if (dist_ind.lt.dist) then
              dist = dist_ind
            endif
          endif
        enddo
      ...
      dist_avg = dist_sum / n_stns_loop

      return
      end subroutine get_forcing_dx
