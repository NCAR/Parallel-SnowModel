...
!      include 'snowmodel.inc'
  ...
  real uwind_grid(nx,max_l_ny)  ! output, E-W wind component
  real vwind_grid(nx,max_l_ny)  ! output, N-S wind component
  real windspd_grid(nx,max_l_ny)
  real winddir_grid(nx,max_l_ny)
  real Qsi_grid(nx,max_l_ny)    ! output
  real Qli_grid(nx,max_l_ny)    ! output
  real prec_grid(nx,max_l_ny)   ! output
  real sprec(nx,max_l_ny)   ! output
  real sfc_pressure(nx,max_l_ny)
  ...
  real terrain_slope(nx,max_l_ny) ! terrain slope
...
! c   terrain.  J. Hydrology, 190, 214-251.
  ...
  use caf_module, only: max_l_ny, l_ny, global_map, me
  use snowmodel_inc
  implicit none
  ...
  real dn                           ! average observation spacing
  real topo(nx,ny)          ! grid topography
  real prec_grid(nx,max_l_ny)     ! output values
  real Tair_grid(nx,max_l_ny)     ! input values
  real sprec(nx,max_l_ny)         ! output values
  real topo_ref_grid(nx,max_l_ny) ! reference surface
...
!c Convert the gridded station data to the SnowModel-grid elevations.
           delta_topo = topo(i,new_j) - topo_ref_grid(i,j)
  ...
  do j=1,l_ny
     ...
     do i=1,nx
        if (icorr_factor_index(iter).gt.0) then
           prec_grid(i,j) = &
                &        corr_factor(i,new_j,icorr_factor_index(iter)) * &
                &        prec_grid(i,j)
        endif
     enddo
  enddo
...
!c Compute the cloud fraction.
        call get_cloudfrac(Tair_grid(i,j),rh_grid(i,j),topo(i,new_j),&
...
!c Compute Qli following Iziomon et al. (2003).
        if (topo(i,new_j).lt.E1) then
           Xs = X1
           Ys = Y1
           Zs = Z1
        elseif (topo(i,new_j).gt.E2) then
           Xs = X3
           Ys = Y3
           Zs = Z3
        else
           Xs = X1 + (topo(i,new_j) - E1) * (X3 - X1)/(E3 - E1)
           Ys = Y1 + (topo(i,new_j) - E1) * (Y3 - Y1)/(E3 - E1)
           Zs = Z1 + (topo(i,new_j) - E1) * (Z3 - Z1)/(E3 - E1)
        endif
...
!      include 'snowmodel.inc'
  ...
  real dn                  ! average observation spacing
  real uwind_grid(nx,max_l_ny)  ! output values
  real vwind_grid(nx,max_l_ny)  ! output values
  real u_grid(nx,max_l_ny)  ! temporary u wind component
  real v_grid(nx,max_l_ny)  ! temporary v wind component
  real winddir_grid(nx,max_l_ny)  ! temporary wind direction
  real windspd_grid(nx,max_l_ny)  ! temporary wind speed
...
! c   relative humidity.
  ...
  real topo(nx,ny) ! grid topography
  real Tair_grid(nx,max_l_ny)  ! output values
  real Td_grid(nx,max_l_ny)    ! output values
  real rh_grid(nx,max_l_ny)    ! output values
...
!c Convert these grid values back to the actual gridded elevations.
  do j=1,l_ny
     new_j = global_map(j)
     do i=1,nx
        delta_topo = topo(i,new_j) - topo_ref
...
subroutine topo_mod_winds(nx,ny,winddir_grid,slopewt,curvewt,&
  ...
  real curvature(nx,max_l_ny)
  real windspd_grid(nx,max_l_ny)
  real winddir_grid(nx,max_l_ny)
  real uwind_grid(nx,max_l_ny)
  real vwind_grid(nx,max_l_ny)
  real wind_slope(nx,max_l_ny)!temporary
  real slope_az(nx,max_l_ny)
  real terrain_slope(nx,max_l_ny)
  real vegtype(nx,max_l_ny)
  real vegsnowd_xy(nx,max_l_ny)
  real windwt(nx,max_l_ny)
...
! c Don't do this smoothing if the domain is arbitrarily small.
  if (nx.gt.100 .and. ny.gt.100) then
     do k=1,loops_windwt_smoother
        call smoother9_parallel(nx,ny,windwt)!It involves halo exchange
     enddo
  endif
...
!      include 'snowmodel.inc'
  ...
  integer i,j,nx,ny,inc,new_j,new_j_start,new_j_end

  real pi,rad2deg,deltax,deltay,deltaxy,curve_len_scale,curve_max

  real topo(nx,ny)
  real dzdx(nx,max_l_ny)
  real dzdy(nx,max_l_ny)
  real curvature(nx,max_l_ny)
  real slope_az(nx,max_l_ny)
  real terrain_slope(nx,max_l_ny)
...
!c Compute the curvature.
     do i=1,nx
        do j=1,l_ny
           new_j = global_map(j)
           curvature(i,j) = (4.0 * topo(i,new_j) - &
                &        topo(max(1,i-inc),max(1,new_j-inc)) - &
                &        topo(min(nx,i+inc),min(ny,new_j+inc)) -&
                &        topo(min(nx,i+inc),max(1,new_j-inc)) -&
                &        topo(max(1,i-inc),min(ny,new_j+inc))) /&
                &        (sqrt(2.0) * 16.0 * real(inc) * deltaxy) +&
                &        (4.0 * topo(i,new_j) -&
                &        topo(min(nx,i+inc),new_j) - topo(max(1,i-inc),new_j) -&
                &        topo(i,min(ny,new_j+inc)) - topo(i,max(1,new_j-inc))) /&
...
!c Find dzdx.
     do j=1,l_ny
        new_j = global_map(j)
        dzdx(1,j) = (topo(2,new_j) - topo(1,new_j)) / deltax
        do i=2,nx-1
           dzdx(i,j) = (topo(i+1,new_j) - topo(i-1,new_j)) / (2.0 * deltax)
        enddo
        dzdx(nx,j) = (topo(nx,new_j) - topo(nx-1,new_j)) / deltax
     enddo

     call new_j_start_end(new_j_start,new_j_end)

!c Find dzdy.
     do i=1,nx
        if(me == 0) dzdy(i,1) = (topo(i,2) - topo(i,1)) / deltay
!        do j=2,ny-1
        do j = new_j_start, new_j_end
           new_j = global_map(j)
           dzdy(i,j) = (topo(i,new_j+1) - topo(i,new_j-1)) / (2.0 * deltay)
        enddo
        if(me == np-1) dzdy(i,l_ny) = (topo(i,ny) - topo(i,ny-1)) / deltay
     enddo
...
!c Compute the average station pressure (in Pa).
  do j=1,l_ny
     new_j = global_map(j)
     do i=1,nx
        sfc_pressure(i,j) = one_atmos * exp((- topo(i,new_j))/scale_ht)
     enddo
  enddo
